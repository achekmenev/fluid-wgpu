import { CellType, Fluid } from './fluid.js'
...
export function allocateIC(numX: number, numY: number): InitialConditions {
...
function isIndexOOB(i: number, j: number): boolean {
  return (
    i < 0 || i >= config.numX ||
    j < 0 || j >= config.numY
  );
...
// Auxiliary function
function discreteToNormalized(ix: number, iy: number): readonly [number, number] {
...
function setBorder(ic: InitialConditions, type: CellType, thickness: number = minBorderThickness) {
...
export function emptyTank(borderThickness: number = minBorderThickness): InitialConditions {
...
export function blowInTank(): InitialConditions {
  const ic = emptyTank();
...
export function flowOverBackwardFacingStepNoSlip(): InitialConditions {
  return flowOverBackwardFacingStep(CellType.SolidNoSlip);
}

export function flowOverBackwardFacingStepFreeSlip(): InitialConditions {
  return flowOverBackwardFacingStep(CellType.SolidFreeSlip);
}

function flowOverBackwardFacingStep(solidType: CellType = CellType.SolidNoSlip): InitialConditions {
...
export function twoFlows(solidType: CellType = CellType.SolidNoSlip): InitialConditions {
  const borderThickness = minBorderThickness;
  const ic = emptyTank();
...
export function twoCounterFlows(solidType: CellType = CellType.SolidNoSlip): InitialConditions {
  const borderThickness = minBorderThickness;
  ...
  for (let i = 0, yi = 0; yi < config.numY; ++yi) {
    for (let xi = 0; xi < config.numX; ++xi, ++i) {
      const [x, y] = discreteToNormalized(xi, yi);
      if (
        (xi < borderThickness && -holeThickness <= y && y <= holeThickness) ||
...
export function flowPastObstacleFreeSlip(): InitialConditions {
  return flowPastObstacle(CellType.SolidFreeSlip);
}

export function flowPastObstacleNoSlip(): InitialConditions {
  return flowPastObstacle(CellType.SolidNoSlip);
}

function flowPastObstacle(solidType: CellType = CellType.SolidFreeSlip): InitialConditions {
  const borderThickness = minBorderThickness;
  const ic = emptyTank(borderThickness);
  //setBorder(ic, CellType.Outflow, borderThickness);
...
function isInternalU(ic: InitialConditions, i: number, j: number): boolean {
  return (
    !isIndexOOB(i, j) && !isIndexOOB(i - 1, j) &&
    ic.b[cellIndex(i, j)] == CellType.Fluid &&
    ic.b[cellIndex(i - 1, j)] == CellType.Fluid
  );
}

function isInternalV(ic: InitialConditions, i: number, j: number): boolean {
  return (
    !isIndexOOB(i, j) && !isIndexOOB(i, j - 1) &&
    ic.b[cellIndex(i, j)] == CellType.Fluid &&
    ic.b[cellIndex(i, j - 1)] == CellType.Fluid
  );
}
export function uniformFlow(): InitialConditions {
...
function joukowsky(x: number, y: number): [number, number] {
 ...
 */
export function isInsideJoukowsky(
    x: number,
    y: number,
    cRe: number = -0.1,
    cIm: number = 0
...
export function joukowskyAirfoil(solidType: CellType = CellType.SolidFreeSlip): InitialConditions {
  const borderThickness = minBorderThickness;
  const ic = emptyTank(borderThickness);
  //setBorder(ic, CellType.Outflow, borderThickness);
