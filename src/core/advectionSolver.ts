...
export class AdvectionSolver {
  private constructor(
    private fluid: Fluid,
    private pipelineVel: GPUComputePipeline,
    private bindGroupsVel: [GPUBindGroup, GPUBindGroup],
    private advectAndEmitDustPipeline: GPUComputePipeline,
    private advectAndEmitDustBindGroups: GPUBindGroup[],
  ) { }
  ...
  static async create(device: GPUDevice, fluid: Fluid) {
    ...
    const advectVelocityPipeline = await PipelineBuilder.createComputePipeline(
      device,
      './core/advectionSolver.wgsl',
      {
    ...
    const advectVelocityBindGroups = [0, 1].map(i => createBindGroup(
      device,
    ...
    const advectAndEmitDustPipeline = await PipelineBuilder.createComputePipeline(
      device,
      './core/advectionSolver.wgsl',
      {
        workgroupSizeX: simCfg.workgroupSizeX,
    ...
    let advectAndEmitDustBindGroups: GPUBindGroup[] = [];
    ...
    return new AdvectionSolver(
      fluid,
      advectVelocityPipeline,
      advectVelocityBindGroups,
  ...
  advectVelocity(commandEncoder: GPUCommandEncoder): void {
  ...
  advectAndEmitDust(commandEncoder: GPUCommandEncoder): void {
    const pass = commandEncoder.beginComputePass();
