...
export class MinMaxReducer {
  ...
  private levelSizes: number[] = [];          // input size (elements) for each level
    // 2DO: make getter of last buffer?
  public intermediateBuffers: GPUBuffer[] = []; // each holds vec2<f32> (min,max) per workgroup
  ...
  constructor(device: GPUDevice, inputBuffer: GPUBuffer, elementCount: number) {
    this.device = device;
  ...
  private createIntermediateBuffers(): void {
  ...
  computeMinMax(commandEncoder: GPUCommandEncoder): void {
